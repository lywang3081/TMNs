...
from networks import net_TMNs_imnet as model
from approaches import TMNs_imnet as approach
import os
import random
import argparse
import shutil
...
parser.add_argument('--dataset', default='imnet', type=str, required=False, choices=['imagenet'], help='Dataset name')
parser.add_argument('--cfg_file', default=None, type=str, required=False, help='Path to the configuration file')
cfg = parser.parse_args()

cfg_file = 'cfg/imagenet.yml'
...
netC = model.netC(2048, n_classes=10, device=cuda1)
...
for t in range(5):
    ...
    dataloader = torch.utils.data.DataLoader(
        ...
        num_workers=int(
            opt.workers))
    test_acc_task, conf_matrixes_task, mask_G = appr.train(dataloader, dataset, t, smax_g=opt.smax_g, use_aux_G=opt.aux_G)
